...
devices = check_output(['xinput', '--list', '--name-only']).decode("utf-8").splitlines()

touchscreen_names = ['touchscreen', 'wacom']
touchscreens = ["pointer:" + i for i in devices if any(j in i.lower() for j in touchscreen_names)]
